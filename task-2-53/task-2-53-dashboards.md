...
**Observations:**
- CPU Utilisation: **5.42%**
- CPU Requests Commitment: **287%**
- CPU Limits Commitment: **311%**
- Memory Utilisation: **21.5%**
- Memory Requests Commitment: **178%**
- Memory Limits Commitment: **181%**

**High usage namespaces:**
- `akash-c-a-monitoring`: **0.243 CPU** and **4.94 GiB** Memory (dominant consumer)

### 3.2 Kubernetes / Compute Resources / Namespace (Pods)

**Namespace Selected:** `akash-c-a-monitoring`

**Observations:**
- CPU Utilisation (from requests): **178%**
- CPU Utilisation (from limits): **1065%**
- Memory Utilisation (from requests): **1529%**
- Memory Utilisation (from limits): **3057%**

**Pod using significantly less CPU than requested:**  
`alertmanager-akash-prometheus-stack-kub-alertmanager-0` (CPU Requests % ≈ 17.9%)
...
**Current Rate:**
- `akash-c-a-monitoring` shows the highest network traffic: **1.09 MiB/s Received** and **173 KiB/s Transmitted**.

### 4.2 Kubernetes / Networking / Namespace (Pods) & Pod

**Namespace:** `akash-c-a-monitoring`  
**Pod:** `prometheus-akash-prometheus-stack-kub-prometheus-0`

- Current Receive Bandwidth: **1.04 MB/s**
- Current Transmit Bandwidth: **43.0 kB/s**
...
**Current Status:** No alerts are firing (panels show low/zero activity as of 2026-05-18).

**When an alert is active:**  
These panels would show increased "Alerts Received", non-zero "Firing Alerts", higher notification send rates, and possible latency values.

## Part 6 — Identify Gaps

After exploring all pre-built dashboards, the following gaps were identified:

1. No application/team-specific scoped dashboard (e.g., only `akash-c-a` namespace with business metrics).
2. No combined view of resource metrics + Loki log volume/rate.
3. No single pane showing Resource Usage + Pod Restarts + OOMKilled events together.
4. Missing golden signals (Latency, Error Rate, QPS/Throughput) for custom applications.
